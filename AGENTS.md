...
- Architecture:
    - `firefly_iii_mcp/api/`: Modular folder for target service client wrappers.
    - `firefly_iii_mcp/mcp/`: Modular folder for action-routed dynamic MCP tool tags.
    - `firefly_iii_mcp/mcp_server.py`: Main MCP server entry point and tool registration.
    - `firefly_iii_mcp/agent_server.py`: Pydantic AI agent definition and logic.
...
- MCP Entry Point → `firefly_iii_mcp/mcp_server.py`
- Agent Entry Point → `firefly_iii_mcp/agent_server.py`
- Source Code → `firefly_iii_mcp/`
- API client mixins → `firefly_iii_mcp/api/`
- MCP tool modules → `firefly_iii_mcp/mcp/`
- Tests → `tests/`
...
edit the canonical `agent-packages/<repo>` checkout** — a
...
git -C agent-packages/<repo> worktree add <external-worktree-root>/<repo>/<branch> -b <branch>
```

Work in an external worktree and **commit often** (commits survive a working-tree reset).
Each session must use a **distinct branch** — git allows a branch in only one
worktree, which is what keeps concurrent sessions from colliding. Keep worktrees
outside the managed workspace so the repository scan leaves them alone.
